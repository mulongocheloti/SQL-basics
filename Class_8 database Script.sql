...
SELECT COALESCE(Town, 'Grand Total') AS Town, SUM(Position*(1+(Prize/100))) AS nv_Total, COUNT(*) AS Occurence FROM Student
	GROUP BY Town WITH ROLLUP
    ORDER BY (CASE WHEN Town IS NULL THEN 1 ELSE 2 END) DESC, nv_Total DESC;
...
SELECT DATE_FORMAT(DateOfBirth, "%D %M %Y") AS DoB FROM STUDENT;

SELECT CONCAT("REGNO: ",REGNO," , ",STUD_NAME) AS "Student info" FROM STUDENT;  
